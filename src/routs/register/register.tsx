...
export default function RegisterSite() {
    ...
    const handleUsernameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const isValid: boolean = usernameValidPattern.test(e.target.value) && e.target.value.length > 3;
    ...
    const handlePassword1Change = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const isValid: boolean = passwordValidPattern.test(e.target.value) && e.target.value.length > 7 && (password2 == e.target.value || password2 == "");
        addErrorClass(e.target, isValid);
    };

    const handlePassword2Change = (e: React.ChangeEvent<HTMLInputElement>) => {
        setPassword2(e.target.value);
        const isValid: boolean = passwordValidPattern.test(e.target.value) && e.target.value.length > 7 && password1 == e.target.value;
        addErrorClass(e.target, isValid);
    };
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if(
            usernameValidPattern.test(username) && username.length > 3 &&
            emailValidPattern.test(email) && email.length > 5 &&
            passwordValidPattern.test(password1) && password1.length > 7 &&
            passwordValidPattern.test(password2) && password1.length > 7 &&
            password1 == password2
        ){
            setErrorData("");
